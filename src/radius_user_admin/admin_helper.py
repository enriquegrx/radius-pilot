...
class Store:
        ...
        with self.lock_path.open("w") as lock:
            ...
            if operation == "create":
                ...
                if operation == "rename":
                    ...
                    user["updated_at"] = now()
                elif operation == "set-device-admin":
                    enabled = self._clean_bool(
                        payload.get("device_admin"), "device administration"
                    )
                    if enabled and "nt_password" not in user:
                        raise AdminError(
                            "Migrate this account to an NT hash before granting "
    ...
    def _render_device_admin(self, data: dict[str, Any]) -> str:
        """Render FreeRADIUS `files` entries for device-administration logins
        (router/switch CLI). Each device-admin account authenticates with its
        existing NT hash — PAP verifies against it — and receives a Cisco shell
        privilege level. This file is consumed by a separate device-admin virtual
        server, completely independent of the VPN authorize file."""
        lines = [
            "# Generated by radius-user-admin. Device administration logins.",
            "# Do not edit by hand.",
        ]
        for user in sorted(data["users"], key=lambda item: item["username"]):
            if not user.get("device_admin") or not self._effective_enabled(user):
                continue
            if "nt_password" not in user:
                continue  # device-admin PAP needs the NT hash
            password_hash = clean_nt_password(user["nt_password"])
            lines.append(f'{user["username"]} NT-Password := 0x{password_hash}')
            lines.append("    Service-Type = NAS-Prompt-User,")
